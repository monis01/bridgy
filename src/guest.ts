...
// GUEST (Child Iframe)
// =============================================================================
// The Guest is the child iframe that initiates the connection.
// It sends SYN to the Host (parent) and waits for SYN_ACK.
// =============================================================================
...
import { createGuestHandshake, type GuestHandshakeController } from './handshake';
...
export class Guest implements BridgeAPI {
  // Configuration
  ...
  // Handshake
  private handshakeController: GuestHandshakeController | null = null;
  ...
  constructor(config: GuestConfig) {
    ...
    this.logger = createLogger('Guest', config.debug ?? false);
  ...
  private getParentWindow(): Window | null {
  ...
  private startHandshake(): void {
    if (!this.parentWindow) {
      const error = new Error('No parent window found. Guest must be in an iframe');
    ...
    this.handshakeController = createGuestHandshake({
  ...
  private sendPacket(packet: BridgePacket): void {
    if (!this.parentWindow) {
      this.logger.warn('Cannot send: no parent window');
      return;
    }
